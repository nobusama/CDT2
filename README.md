...
- Supplementary Data S2 — per-community GO and pathway (Enrichr) enrichment results for both attention mechanisms (`data/supp_data_S2_community_enrichment.csv`) — is included in the repository. Columns: mechanism, community ID, manuscript label (C1/C3), community size, library, term, overlap, raw and adjusted *P*-values, odds ratio, combined score, and member genes. Significant terms only (adjusted *P* < 0.05) across the same libraries used in the manuscript Methods — GO Molecular Function, KEGG, and Reactome; communities with no significant terms are omitted, and two communities with fewer than five genes were not tested. These results were recomputed from the S1 gene lists via Enrichr (gseapy) on 2026-06-27 using the GO_Molecular_Function_2023, KEGG_2021_Human, and Reactome_2022 libraries against the default genome-wide background; values may differ slightly from those quoted in the manuscript text, which were computed on an earlier library release.
...
CDT v1 ([arXiv:2601.01089](https://arxiv.org/abs/2601.01089)) established the foundational framework for integrating all three Central Dogma modalities: DNA, RNA, and protein. CDT-II focused on DNA and RNA to develop cell-level interpretable architecture. The next generation aims to extend CDT-II's cell-level interpretable architecture to all three modalities, building on CDT v1's foundational framework:
